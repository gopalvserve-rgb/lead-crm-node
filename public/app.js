...
const NAV_GROUPS = [
  ...
  { label: 'Reports', icon: '📉', items: [
    ...
    /* CALL_ACTIVITY_v1 */ { id: 'callactivity', label: 'Call activity', icon: '📞', roles: ['admin', 'manager', 'team_leader'] },
...
VIEWS.leads = async (view) => {
  ...
  const toolbar = h('div', { class: 'toolbar' },
    ...
    dateRangeWrap,
    multiSelectDropdown({
      label: 'Status', allLabel: 'Any status',
      options: statuses.map(s => ({ id: String(s.id), name: s.name })),
      values: (Array.isArray(CRM.prefs.filters.status_ids) && CRM.prefs.filters.status_ids.length) ? CRM.prefs.filters.status_ids.map(String) : (CRM.prefs.filters.status_id ? [String(CRM.prefs.filters.status_id)] : []),
      onApply: (v) => { CRM.prefs.filters.status_ids = v; CRM.prefs.filters.status_id = v.length === 1 ? v[0] : ''; applyFilters(); }
    }),
    multiSelectDropdown({
      label: 'Source', allLabel: 'Any source',
      options: sources.map(s => ({ id: s.name, name: s.name })),
      values: (Array.isArray(CRM.prefs.filters.sources) && CRM.prefs.filters.sources.length) ? CRM.prefs.filters.sources : (CRM.prefs.filters.source ? [CRM.prefs.filters.source] : []),
      onApply: (v) => { CRM.prefs.filters.sources = v; CRM.prefs.filters.source = v.length === 1 ? v[0] : ''; applyFilters(); }
    }),
    multiSelectDropdown({
      label: 'Assigned', allLabel: 'Any assignee',
      options: users.map(u => ({ id: String(u.id), name: u.name })),
      values: (Array.isArray(CRM.prefs.filters.assigned_tos) && CRM.prefs.filters.assigned_tos.length) ? CRM.prefs.filters.assigned_tos.map(String) : (CRM.prefs.filters.assigned_to ? [String(CRM.prefs.filters.assigned_to)] : []),
      onApply: (v) => { CRM.prefs.filters.assigned_tos = v; CRM.prefs.filters.assigned_to = v.length === 1 ? v[0] : ''; applyFilters(); }
    }),
    multiSelectDropdown({
      label: 'Tag', allLabel: 'Any tag',
      options: _tagOpts,
...
async function loadLeads(opts) {
  ...
  CRM._leadsPage = page;
  // Multi-select arrays now live in CRM.prefs.filters (set by the
  // checkbox-dropdown onApply handlers above). No DOM read needed.
...
function openBulkUpload() {
  ...
  const modal = h('div', { class: 'modal-backdrop' },
    h('div', { class: 'modal modal-lg' },
      ...
      fileInput,
      // THREE_FEATURES_v1 — F1: required-columns panel + sample CSV
      (function(){
      ...
      (function _csvReqsCard(){
        ...
        return h('div', { class: 'card', style: { background: '#f0f9ff', border: '1px solid #bae6fd', marginBottom: '.6rem' } },
          h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.4rem', flexWrap: 'wrap', gap: '.4rem' } },
            h('div', {},
              h('div', { style: { fontWeight: 700, fontSize: '.95rem', color: '#075985' } }, '\ud83d\udccb Required & recommended columns'),
              h('div', { class: 'muted', style: { fontSize: '.78rem' } }, 'Use these exact headers for auto-mapping. Extra columns become custom fields.')
            ),
            h('button', { class: 'btn sm primary', title: 'Download a CSV template', onclick: () => {
          ...
          h('div', { class: 'muted', style: { fontSize: '.74rem', marginTop: '.4rem', borderTop: '1px dashed #bae6fd', paddingTop: '.35rem' } },
            'Tip: leave a column blank if you don\u2019t have a value \u2014 it won\u2019t overwrite an existing field. Phone numbers are deduplicated automatically.'
          )
        );
      })(),
      fileInfo,
...
async function openLeadModal(id) {
  ...
  // Append extra-phones widget at the end of the form so it spans width.
  try { form.appendChild(extraPhonesWrap); } catch (_) {}

  // THREE_FEATURES_v1 — F2: collapsible "All other fields" with every
  // leads-table column not already hardcoded above. Read-only for system
  // fields (created_at, heat_score, etc), editable otherwise.
  try {
    ...
    // Also render registered custom-field keys' values that aren't
    // already shown (e.g. inactive CFs whose values still live in extra).
    const _cfKeys = new Set(((customFields || []).map(c => c.key).filter(Boolean)));
    const moreKeys = [];
    ...
    // Also pick up orphan keys from extra_json that aren't a registered CF
    const _extra = (lead && lead.extra) || {};
    Object.keys(_extra).forEach(k => {
      if (_cfKeys.has(k)) return;
      if (_allFieldsRendered.has(k)) return;
      if (String(k)[0] === '_') return;
      const v = _extra[k];
      if (v == null || typeof v === 'object') return;
      moreKeys.push('extra.' + k);
    });
    if (moreKeys.length) {
      function _humanize(k) {
        return String(k || '').replace(/^extra\./, '').replace(/[_\-]+/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      }
      ...
      moreKeys.forEach(k => {
        const realKey = k.startsWith('extra.') ? k.slice(6) : k;
        const isExtra = k.startsWith('extra.');
        const ro = _readOnlyKeys.has(realKey);
        const v = isExtra ? _extra[realKey] : lead[realKey];
        const isDate = /at$|_date$/.test(realKey) && v && /\d{4}-\d{2}/.test(String(v));
        const wrap = h('label', { class: 'field all-field' + (ro ? ' readonly' : '') + (isExtra ? ' is-extra' : ''), 'data-all-field-key': realKey, 'data-all-field-extra': isExtra ? '1' : '0', style: { display: 'flex', flexDirection: 'column', gap: '.15rem' } });
        wrap.appendChild(h('span', { style: { fontWeight: 600, fontSize: '.82rem' } },
          ...
          isExtra ? h('span', { style: { color: '#f59e0b', fontSize: '.7rem', marginLeft: '.3rem' } }, '(custom)') : null
        ));
        wrap.appendChild(h('input', {
          type: 'text',
          name: (isExtra ? 'cf_' : 'allf_') + realKey,
  ...
  form.addEventListener('submit', async ev => {
    ...
    // THREE_FEATURES_v1 — F2: collect "All other fields" + orphan extras.
    try {
      form.querySelectorAll('.all-field').forEach(wrap => {
        ...
        if (!inp) return;
        if (wrap.getAttribute('data-all-field-extra') === '1') {
          if (!Object.prototype.hasOwnProperty.call(extra, k)) extra[k] = inp.value || '';
        } else {
          // Land on the payload directly so it maps to the leads-table column.
          // (Defined below.) Stash here, merge once payload is built.
          wrap._pendingAll = inp.value;
        }
      });
      // Preserve non-primitive values that the render pass intentionally skipped
    ...
    } catch (e) { console.warn('[F2 submit]', e && e.message); }
...
/**
 * Checkbox-dropdown multi-select (Select All / Deselect All / Cancel / Apply).
 * Ported from smartcrm-saas. Replaces the native <select multiple> in places
 * where users need a real picker instead of Ctrl/Cmd-click trickery.
 *
 * Usage:
 ...
 *   dd.getValues();   // → ['1', '2']
 *   dd.setValues(v);  // programmatic update
 */
...
function buildWaCompose(phone, onSent) {
  ...
  // ↩ Reply on a message.
  const replyPreviewSlot = h('div', { class: 'wb-compose-reply-preview', style: { display: 'none', padding: '.4rem .55rem', background: 'linear-gradient(90deg, #eef2ff, #ffffff)', borderLeft: '3px solid #6366f1', borderRadius: '6px', margin: '0 0 .35rem', fontSize: '.78rem', alignItems: 'flex-start', justifyContent: 'space-between', gap: '.4rem' } });
...
async function wbChat() {
  ...
  async function renderActiveThread(force) {
    ...
    msgs.forEach(msg => {
      ...
      // WA_REPLY_v1: render quoted parent if this message is a reply
...
VIEWS.reports = async (view) => {
  ...
  const filterBar = h('div', { class: 'toolbar' },
    ...
    multiSelectDropdown({
      id: 'rep-user-dd', label: 'Users', allLabel: 'All users',
      options: users.map(u => ({ id: String(u.id), name: u.name })),
    ...
    // ---- Lead-field filters ------------------------------------
    multiSelectDropdown({
      id: 'rep-status-dd', label: 'Status', allLabel: 'Any status',
      options: statuses.map(s => ({ id: String(s.id), name: s.name })),
    ...
    multiSelectDropdown({
      id: 'rep-product-dd', label: 'Product', allLabel: 'Any product',
    ...
    multiSelectDropdown({
      id: 'rep-source-dd', label: 'Source', allLabel: 'Any source',
      options: sources.map(s => ({ id: s.name, name: s.name })),
...
function _currentReportFilters() {
  ...
  const _picked = window._repPicked || {};
  const _userIds   = (_picked.users     || []).map(String);
  const _statusIds = (_picked.statuses  || []).map(String);
  const _prodIds   = (_picked.products  || []).map(String);
  const _srcVals   = (_picked.sources   || []).map(String);
  // Back-compat: when exactly one value is picked, also send the legacy
  // singular key so older backend code that only checks .user / .status_id
  // keeps working.
  const user        = _userIds.length === 1 ? _userIds[0]    : undefined;
  const user_ids    = _userIds.length ? _userIds : undefined;
  const role        = $('#rep-role')?.value || undefined;
  const status_id   = _statusIds.length === 1 ? _statusIds[0] : undefined;
  const status_ids  = _statusIds.length ? _statusIds : undefined;
  const product_id  = _prodIds.length === 1 ? _prodIds[0] : undefined;
  const product_ids = _prodIds.length ? _prodIds : undefined;
  const source      = _srcVals.length === 1 ? _srcVals[0] : undefined;
  const sources     = _srcVals.length ? _srcVals : undefined;
  const qualified = $('#rep-qualified')?.value || undefined;
  const tag = $('#rep-tag')?.value || undefined;
  return { from, to, scope_user_id: user, scope_user_ids: user_ids, role, status_id, status_ids, product_id, product_ids, source, sources, qualified, tag };
}
...
function _currentReportBuilderFilters() {
  const p = window._rbPicked || {};
  const userIds   = (p.users    || []).map(String);
  const statusIds = (p.statuses || []).map(String);
  const prodIds   = (p.products || []).map(String);
  const srcVals   = (p.sources  || []).map(String);
  return {
    from:           $('#rb-from')?.value || undefined,
    to:             $('#rb-to')?.value   || undefined,
    scope_user_id:  userIds.length === 1 ? userIds[0] : undefined,
    scope_user_ids: userIds.length ? userIds : undefined,
    status_id:      statusIds.length === 1 ? statusIds[0] : undefined,
    status_ids:     statusIds.length ? statusIds : undefined,
    product_id:     prodIds.length === 1 ? prodIds[0] : undefined,
    product_ids:    prodIds.length ? prodIds : undefined,
    source:         srcVals.length === 1 ? srcVals[0] : undefined,
    sources:        srcVals.length ? srcVals : undefined,
...
async function adminSmtp() {
  const cfg = await api('api_admin_getConfig');
  const me = (typeof CRM !== 'undefined' && CRM.user) ? CRM.user : {};
  ...
  // SMTP_TEST_v1 — Test SMTP card
  const wrap = h('div', {});
  wrap.appendChild(form);
  const testCard = h('div', { style: { marginTop: '1rem', padding: '14px 16px', border: '1px solid #c7d2fe', background: 'linear-gradient(180deg, #eef2ff 0%, #fafaff 100%)', borderRadius: '12px' } });
  testCard.appendChild(h('div', { style: { fontWeight: 700, fontSize: '.95rem', color: '#3730a3', marginBottom: '4px' } }, '✉️ Test SMTP'));
  testCard.appendChild(h('div', { style: { fontSize: '.82rem', color: '#475569', marginBottom: '10px' } }, 'Send a real test email using the values shown in the form above (you do NOT need to save first). If it fails, the exact SMTP error will be shown so you can fix it.'));
  const toInput = h('input', { type: 'email', placeholder: 'Recipient (e.g. you@example.com)', value: (me && me.email) || '', style: { padding: '8px 10px', border: '1px solid #cbd5e1', borderRadius: '8px', minWidth: '260px', marginRight: '8px' } });
  ...
  testBtn.onclick = async () => {
    ...
    } finally { testBtn.disabled = false; testBtn.textContent = origLabel; }
  };
...
VIEWS.users = async (view) => {
  ...
  view.append(
    h('div', { class: 'toolbar' },
      ...
      // THREE_FEATURES_v1 — F3: hierarchy chart button
...
VIEWS.tasks = async (view) => {
   ...
   * so they can drill into who has done what. */
  ...
  /* CEL_TASKS_BY_EMP_v1 — admin view: who has done what */
...
VIEWS.aibot = async (view) => {
  ...
  function _renderSub() {
    ...
    else if (activeSub === 'usage')    _renderAiBotUsage(subView);
  }
  subTabs.forEach(t => {
...
async function _renderAiBotSettings(host) {
  ...
  // ---- Card 1: Master switch + identity ----
  const c1 = h('div', { class: 'card' });
  c1.appendChild(h('h3', { style: { marginTop: 0 } }, '🤖 Identity'));
  ...
  // langInp replaced by langChks below

  // Multi-language picker — replace single-input with checkbox grid
  const LANG_OPTIONS = [
  ...
  const currentLangs = String(s.language || 'en').split(/[+,\s]/).map(x => x.trim()).filter(Boolean);
  const langChks = {};
  const langGridEl = h('div', {
  ...
  LANG_OPTIONS.forEach(o => {
    ...
    langGridEl.appendChild(h('label', {
  ...
  // Goal templates — pick one to fill the system_prompt below
  ...
  goalSel.addEventListener('change', () => {
    ...
    if (!tpl) return;
    const filled = tpl.prompt.replace(/\{\{BUSINESS_NAME\}\}/g, businessInp.value || 'this business');
    if (promptArea.value && !confirm('Replace the current system prompt with the ' + tpl.label + ' template?')) return;
    promptArea.value = filled;
    if (typeof toast === 'function') toast('✅ Loaded ' + tpl.label + ' template — edit below to customise');
  });

  const promptArea    = h('textarea', { rows: 8, style: { width: '100%' } }, s.system_prompt || '');
  ...
  c1.appendChild(f('Bot name',         botNameInp));
  c1.appendChild(f('Business name',    businessInp));
  c1.appendChild(f('Reply languages',
    langGridEl,
    'Tick all the languages your bot should reply in. Bot detects customer\'s language and matches.'));
  c1.appendChild(f('🎯 Bot goal — preset templates',
    goalSel,
    'Pick a preset to fill the system prompt with a tested template, then edit below to customise.'));
  c1.appendChild(f('System prompt',    promptArea,   'Instructions for the bot. Use {business} and {bot_name} placeholders.'));
  c1.appendChild(f('Welcome message',  welcomeArea,  'Optional auto-reply on first message from a new contact.'));

  // ---- Card 2: Response timing + reply modes ----
  const c2 = h('div', { class: 'card', style: { marginTop: '1rem' } });
  c2.appendChild(h('h3', { style: { marginTop: 0 } }, '⏱ Response timing & modes'));
  const idleSecInp = h('input', { type: 'number', min: '0', max: '604800', value: String(s.resume_after_idle_seconds || 86400), style: { width: '120px' } });
  const maxRepliesInp = h('input', { type: 'number', min: '0', max: '999', value: String(s.max_replies_per_thread || 0), style: { width: '120px' } });
  const triggerInp = h('input', { value: s.trigger_keywords || '', placeholder: 'price, demo, info', style: { width: '100%' } });
  const offInp = h('input', { value: s.off_keywords || '', placeholder: 'human, agent, stop', style: { width: '100%' } });
  const escInp = h('input', { value: s.escalation_keywords || '', placeholder: 'angry, refund, lawyer', style: { width: '100%' } });

  // Reply-mode checkboxes
  const modes = Array.isArray(s.reply_modes) ? s.reply_modes.slice() : ['always'];
  const ALL_MODES = [
    { id: 'always',      label: 'Always reply' },
    { id: 'after_hours', label: 'Only outside business hours' },
    { id: 'keyword',     label: 'Only when trigger keyword matches' },
    { id: 'manual',      label: 'Draft replies for agent to send' },
    { id: 'phone_only',  label: 'Only on selected WhatsApp numbers' }
  ];
  const modeChecks = {};
  const modesWrap = h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '.5rem' } });
  ALL_MODES.forEach(m => {
    const chk = h('input', { type: 'checkbox', checked: modes.includes(m.id) ? 'checked' : null });
    modeChecks[m.id] = chk;
    modesWrap.appendChild(h('label', { style: { display: 'flex', gap: '.3rem', alignItems: 'center', padding: '.35rem .6rem', background: '#f8fafc', borderRadius: '6px', fontSize: '.85rem' } }, chk, m.label));
  });
  c2.appendChild(f('Reply modes', modesWrap, 'Pick one or more — combined with AND. Default: Always reply.'));

  c2.appendChild(f('Resume after idle (seconds)', idleSecInp,
    'After your AGENT replies, the bot pauses for this long before resuming auto-replies. 86400 = 1 day.'));
  c2.appendChild(f('Max replies per thread', maxRepliesInp,
    '0 = unlimited. Caps how many auto-replies the bot sends in a single conversation before handing off to a human.'));
  c2.appendChild(f('Trigger keywords', triggerInp,
    'Comma-separated. Only used when "keyword" reply mode is on. Example: price, demo, info.'));
  c2.appendChild(f('Off keywords', offInp,
    'Bot stays silent if message contains any of these. Useful for "speak to human" type opt-outs.'));
  c2.appendChild(f('Escalation keywords', escInp,
    'Bot drafts a reply (instead of auto-sending) when message contains any of these. Useful for refund/legal/complaint flows.'));

  // ---- Card 3: Business hours ----
  const c3 = h('div', { class: 'card', style: { marginTop: '1rem' } });
  c3.appendChild(h('h3', { style: { marginTop: 0 } }, '🕘 Business hours'));
  const bh = (s.business_hours && typeof s.business_hours === 'object') ? s.business_hours : { tz: 'Asia/Kolkata', days: [1,2,3,4,5], start: '09:00', end: '19:00' };
  const tzInp = h('input', { value: bh.tz || 'Asia/Kolkata', style: { width: '180px' } });
  const startInp = h('input', { type: 'time', value: bh.start || '09:00', style: { width: '120px' } });
  const endInp   = h('input', { type: 'time', value: bh.end   || '19:00', style: { width: '120px' } });
  const DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const dayChecks = DAYS.map((d, i) => {
    const chk = h('input', { type: 'checkbox', checked: (bh.days || []).includes(i) ? 'checked' : null, 'data-day': String(i) });
    return { i, label: d, chk };
  });
  const daysWrap = h('div', { style: { display: 'flex', gap: '.4rem', flexWrap: 'wrap' } },
    ...dayChecks.map(d => h('label', { style: { display: 'flex', gap: '.25rem', alignItems: 'center', padding: '.3rem .5rem', background: '#f8fafc', borderRadius: '6px', fontSize: '.85rem' } }, d.chk, d.label))
  );
  c3.appendChild(f('Timezone',  tzInp,    'IANA name. Asia/Kolkata = IST.'));
  c3.appendChild(f('Working days', daysWrap));
  c3.appendChild(h('div', { style: { display: 'flex', gap: '.5rem', alignItems: 'center', marginBottom: '.6rem' } },
    h('label', { style: { fontWeight: '500', fontSize: '.85rem' } }, 'Start'), startInp,
    h('label', { style: { fontWeight: '500', fontSize: '.85rem', marginLeft: '.5rem' } }, 'End'), endInp
  ));

  // ---- Card 4: KB usage controls ----
  const c4 = h('div', { class: 'card', style: { marginTop: '1rem' } });
  c4.appendChild(h('h3', { style: { marginTop: 0 } }, '📚 Knowledge & memory'));
  ...
  c4.appendChild(f('',
    h('label', { style: { display: 'flex', alignItems: 'center', gap: '.4rem' } },
      useKbChk, h('span', {}, 'Use Knowledge Base for grounding (recommended)')
    )
  ));
  c4.appendChild(f('KB context cap (chars)', kbMaxInp, 'Max characters of KB sent with every request. 60000 ≈ 10k tokens.'));
  c4.appendChild(f('History messages',       histInp,  'Last N messages of the WhatsApp thread fed to Gemini for context.'));

  // ---- Save row ----
  const status = h('div', { style: { fontSize: '.85rem', marginTop: '.5rem' } });
  const saveRow = h('div', { class: 'card', style: { marginTop: '1rem', display: 'flex', gap: '.5rem', alignItems: 'center' } });
  saveRow.appendChild(h('button', { class: 'btn primary', onclick: async () => {
    try {
      const reply_modes = ALL_MODES.filter(m => modeChecks[m.id].checked).map(m => m.id);
      if (!reply_modes.length) reply_modes.push('always');
      const days = dayChecks.filter(d => d.chk.checked).map(d => d.i);
      await api('api_aibot_settings_save', {
        ...
        reply_modes,
        business_hours: { tz: tzInp.value, days, start: startInp.value, end: endInp.value },
        trigger_keywords: triggerInp.value,
        off_keywords: offInp.value,
        escalation_keywords: escInp.value,
        resume_after_idle_seconds: Number(idleSecInp.value || 86400),
        max_replies_per_thread: Number(maxRepliesInp.value || 0),
...
async function _renderAiBotKb(host) {
  ...
  // ---- Crawl website URL ----
  const urlInp = h('input', { placeholder: 'https://yoursite.com/pricing', style: { width: '100%', marginBottom: '.4rem' } });
  const crawlStatus = h('div', { style: { fontSize: '.85rem', marginTop: '.4rem' } });
  card.appendChild(h('div', { style: { padding: '.6rem', background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: '6px', marginBottom: '.75rem' } },
    h('div', { style: { fontWeight: '600', marginBottom: '.3rem' } }, '🌐 Crawl website URL'),
    h('div', { class: 'muted', style: { fontSize: '.78rem', marginBottom: '.4rem' } }, 'Paste any public page URL — we fetch the HTML, strip tags, and store the visible text as a KB document.'),
    urlInp,
    h('div', { style: { display: 'flex', gap: '.5rem' } },
      h('button', { class: 'btn', onclick: async () => {
...
// ============================================================
// ✨ CRM Copilot — floating Ask CRM widget
// ============================================================
...
function _renderCopilotDrawer() {
  ...
  head.appendChild(h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } },
    h('div', {},
      h('b', {}, '✨ Ask CRM'),
      h('div', { style: { fontSize: '.75rem', opacity: '.85', marginTop: '.15rem' } }, 'Gemini-powered — reads your CRM data'),
  ...
  log.appendChild(_copilotMsg('model',
    'Hi ' + ((CRM.user && CRM.user.name) || '').split(' ')[0] + '! Ask me anything about your CRM data — like:\n' +
    '• How many leads in total?\n' +
    '• Top 3 performers this month\n' +
    '• Tell me about lead <name>\n' +
    '• Conversion by source\n' +
    '• Who came late today?\n' +
    '• Unattended WhatsApp chats'
  ));
...
function openUserHierarchyModal(users) {
  ...
  if (!list.length) { if (typeof toast === 'function') toast('No users to chart', 'warn'); return; }
  const byId = {};
  ...
  const NODE_W = 180, NODE_H = 64, H_GAP = 24, V_GAP = 50;
  function _layout(n, d) {
    n.depth = d;
    if (!n.children.length) { n.width = NODE_W; return NODE_W; }
    let total = 0; n.children.forEach(c => { total += _layout(c, d + 1); });
    total += H_GAP * (n.children.length - 1);
    n.width = Math.max(NODE_W, total);
    return n.width;
  }
  function _assignX(n, x0) {
    n.x = x0 + (n.width - NODE_W) / 2;
    n.y = 30 + n.depth * (NODE_H + V_GAP);
    let cx = x0;
    n.children.forEach((c, i) => { if (i > 0) cx += H_GAP; _assignX(c, cx); cx += c.width; });
  }
  let totalW = 0; roots.forEach(r => { totalW += _layout(r, 0); });
  let cx = 0; roots.forEach((r, i) => { if (i > 0) cx += H_GAP * 2; _assignX(r, cx); cx += r.width; });
  let maxX = 0, maxY = 0;
  list.forEach(u => { const n = byId[u.id]; if (n.x == null) return; if (n.x + NODE_W > maxX) maxX = n.x + NODE_W; if (n.y + NODE_H > maxY) maxY = n.y + NODE_H; });
  const SVG_W = Math.max(maxX + 30, 600), SVG_H = Math.max(maxY + 30, 200);
  const ns = 'http://www.w3.org/2000/svg';
  const svg = document.createElementNS(ns, 'svg');
  svg.setAttribute('viewBox', '0 0 ' + SVG_W + ' ' + SVG_H);
  svg.setAttribute('width', SVG_W); svg.setAttribute('height', SVG_H);
  svg.style.maxWidth = '100%'; svg.style.background = '#fff';
  list.forEach(u => {
    const n = byId[u.id];
    if (!n.children || !n.children.length) return;
    const pcx = n.x + NODE_W / 2, pcy = n.y + NODE_H;
    n.children.forEach(c => {
      const ccx = c.x + NODE_W / 2, ccy = c.y, midY = (pcy + ccy) / 2;
      const path = document.createElementNS(ns, 'path');
      path.setAttribute('d', 'M' + pcx + ',' + pcy + ' V' + midY + ' H' + ccx + ' V' + ccy);
      path.setAttribute('fill', 'none'); path.setAttribute('stroke', '#cbd5e1'); path.setAttribute('stroke-width', '2');
  ...
  list.forEach(u => {
    ...
    g.addEventListener('click', () => { m.remove(); if (typeof openUserModal === 'function') openUserModal(u.id); });
    const rect = document.createElementNS(ns, 'rect');
    rect.setAttribute('width', NODE_W); rect.setAttribute('height', NODE_H); rect.setAttribute('rx', 8);
    rect.setAttribute('fill', c.bg); rect.setAttribute('stroke', c.border); rect.setAttribute('stroke-width', '2');
    g.appendChild(rect);
    const name = document.createElementNS(ns, 'text');
    name.setAttribute('x', NODE_W / 2); name.setAttribute('y', 24); name.setAttribute('text-anchor', 'middle');
    name.setAttribute('font-family', 'Inter, Arial, sans-serif'); name.setAttribute('font-size', '13'); name.setAttribute('font-weight', '700');
    name.setAttribute('fill', c.text); name.textContent = (u.name || '').slice(0, 24);
    g.appendChild(name);
    const role = document.createElementNS(ns, 'text');
    role.setAttribute('x', NODE_W / 2); role.setAttribute('y', 42); role.setAttribute('text-anchor', 'middle');
    role.setAttribute('font-family', 'Inter, Arial, sans-serif'); role.setAttribute('font-size', '10');
    role.setAttribute('fill', c.text); role.setAttribute('opacity', '0.85'); role.textContent = (u.role || '').toUpperCase();
    g.appendChild(role);
    const dept = document.createElementNS(ns, 'text');
    dept.setAttribute('x', NODE_W / 2); dept.setAttribute('y', 56); dept.setAttribute('text-anchor', 'middle');
    dept.setAttribute('font-family', 'Inter, Arial, sans-serif'); dept.setAttribute('font-size', '10');
    dept.setAttribute('fill', '#475569'); dept.textContent = (u.department || '').slice(0, 28);
    g.appendChild(dept);
    svg.appendChild(g);
  });
  const m = h('div', { class: 'modal-backdrop', onclick: ev => { if (ev.target.classList.contains('modal-backdrop')) m.remove(); } });
